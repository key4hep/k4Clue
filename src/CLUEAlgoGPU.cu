...
__global__ void kernel_compute_histogram( TILES &d_hist, 
...
void CLUEAlgoGPU_T<TILES>::makeClusters( ) {
  ...
  kernel_find_clusters<<<gridSize,blockSize>>>(d_seeds, d_followers, d_points,
					       CLUEAlgo_T<TILES>::outlierDeltaFactor_, CLUEAlgo_T<TILES>::dc_, CLUEAlgo_T<TILES>::rhoc_,
					       CLUEAlgo_T<TILES>::points_.n);
  ...
  kernel_assign_clusters<<<gridSize_nseeds,blockSize>>>(d_seeds, d_followers, d_points, CLUEAlgo_T<TILES>::points_.n);

  copy_tohost();
}

// explicit template instantiation
template class CLUEAlgoGPU_T<LayerTilesGPU>;
