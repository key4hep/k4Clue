  ...
  for (size_t i = 0; i < points_.n; i++) {
    ...
    for (int xBin = search_box[0]; xBin <= search_box[1]; ++xBin) {
      for (int yBin = search_box[2]; yBin <= search_box[3]; ++yBin) {
        ...
        //        std::cout << "binSize = " << binSize << " for [xBin,yBin] = [" << xBin << "," << yBin << "]" <<
        //        std::endl;
        ...
        for (size_t binIter = 0; binIter < binSize; binIter++) {
          ...
          // query N_{dc_}(i)
          float dist2_ij = TILES::constants_type_t::endcap ? distance2(i, j) : distance2(i, j, true, ri);
          if (dist2_ij <= dc2) {
  ...
  for (size_t i = 0; i < points_.n; i++) {
    ...
    for (int xBin = search_box[0]; xBin <= search_box[1]; ++xBin) {
      for (int yBin = search_box[2]; yBin <= search_box[3]; ++yBin) {
        ...
        int phi = (xBin % TILES::constants_type_t::nColumnsPhi);
        int binId =
            TILES::constants_type_t::endcap ? lt.getGlobalBinByBin(xBin, yBin) : lt.getGlobalBinByBinPhi(phi, yBin);
        ...
        for (int binIter = 0; binIter < binSize; binIter++) {
          ...
          float dist_ij = TILES::constants_type_t::endcap ? distance(i, j) : distance(i, j, true, ri);
